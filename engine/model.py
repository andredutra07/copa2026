...
# -*- coding: utf-8 -*-
"""
engine/model.py — núcleo do Previsor Copa 2026 (importável, sem efeitos no import).

Dados ESTÁTICOS (times, Elo, calendário, R32) ficam aqui.
Dados DINÂMICOS (placares, xG, lesões, parâmetros calibrados) são lidos de data/*.json,
escritos pelo run_update.py (que roda na GitHub Action).
"""
...
TITLE_ODDS = {  # odds americanas de título (mercado) -> entram por de-vig
...
GLET = sorted(GROUPS)
ALT_VENUE_TEAM = {'México'}  # jogos do México em casa = Cidade do México (altitude)
...
MIN_FIT = 24  # congela parâmetros até 24 jogos (anti-overfit)
...
def load_results():
    """Resultados reais. Lista de [home, away, gh, ga, (xgh, xga opcional)]."""
    return _load("results.json", [
        ["México","África do Sul",2,0],["Coreia do Sul","Tchéquia",2,1],["Canadá","Bósnia",1,1],
        ["Estados Unidos","Paraguai",4,1],["Catar","Suíça",1,1],["Brasil","Marrocos",1,1],
        ["Haiti","Escócia",0,1],["Austrália","Turquia",2,0],
    ])

def load_adjustments():
    """Ajustes por time (att/def) vindos de xG e lesões. {team: {'att':d,'def':d,'reason':str}}"""
...
def log_opinion_pool(p_model, p_market, w):
    pm=np.clip(p_model,1e-9,1); mk=np.clip(p_market,1e-9,1)
    f=pm**(1-w)*mk**w; return f/f.sum()
...
def elo_market_prior():
    elos=np.array([TEAMS[t]['elo'] for t in NAMES],float); z=(elos-elos.mean())/elos.std()
    s={t:0.18*zi for t,zi in zip(NAMES,z)}
    tp=devig_title_probs()
    if tp:
        lt={t:math.log(p) for t,p in tp.items()}; v=np.array(list(lt.values()))
        lz={t:(lt[t]-v.mean())/v.std() for t in lt}; W=0.35
        for t in lz: s[t]=(1-W)*s[t]+W*(0.18*lz[t])
    return s
...
def init_params(adjust=None):
    ...
    if adjust:
        for t,d in adjust.items():
            if t in p.att:
                p.att[t]+=d.get('att',0.0); p.dfn[t]-=d.get('def',0.0)  # lesão defensiva -> defesa pior
    return p
...
def predict(p,h,a,neutral=True,altitude=False):
    ...
    # placar mais provável orientado ao vencedor previsto (corrige o bug da auditoria)
...
def light_update(prior, results, lr=0.16, K0=3.0, cap=0.22):
    """Aprendizado em tempo real (empirical-Bayes): ajusta att/def de cada seleção
       pelos gols marcados/sofridos vs esperado, com forte encolhimento (K0) e teto
       (cap). Roda enquanto há poucos jogos para o MLE completo — pega forma sem viciar."""
    import collections
    sc=collections.defaultdict(float); cc=collections.defaultdict(float); gp=collections.defaultdict(int)
    for r in results:
        ...
        sc[h]+=(gh-lh)/(lh+1.0); cc[h]+=(ga-la)/(la+1.0)
        sc[a]+=(ga-la)/(la+1.0); cc[a]+=(gh-lh)/(lh+1.0)
        gp[h]+=1; gp[a]+=1
    ...
    for t in NAMES:
        n=gp.get(t,0)
        if n:
            p.att[t]+=max(-cap,min(cap, lr*sc[t]/(n+K0)))
            p.dfn[t]+=max(-cap,min(cap, lr*(-cc[t])/(n+K0)))
    return p

def current_params(results, adjust=None):
    """Parâmetros vigentes: prior (Elo+mercado+ajuste manual) -> MLE se >=MIN_FIT,
       senão aprendizado leve a partir dos jogos."""
...
def simulate(p, results, N=20000):
    ...
    for _ in range(N):
        ...
        slot_group=annex_c.allocate(sorted(third_team))  # (i,j)->grupo, via Annex C
        bracket=[]
...
def project_display(p, results):
    """Projeção determinística (caminho mais provável) para exibição:
       tabela dos grupos por pontos esperados + bracket round-a-round com placar
       orientado ao vencedor. Não substitui o Monte Carlo (que dá as probabilidades)."""
    fixed=played_map(results)
    def exp_points(L):
        pts={t:0.0 for t in GROUPS[L]}; gd={t:0.0 for t in GROUPS[L]}
        for h,a in itertools.combinations(GROUPS[L],2):
            if (h,a) in fixed: gh,ga=fixed[(h,a)]; A,B=h,a
            elif (a,h) in fixed: ga,gh=fixed[(a,h)]; A,B=h,a
            else:
...
def build_predictions(N=20000):
    ...
    tp=devig_title_probs()
    done={(r[0],r[1]) for r in results}|{(r[1],r[0]) for r in results}
    ...
    champ_list=sorted(([t,champ[t]/n,wilson_ci(champ[t],n)] for t in NAMES if champ[t]>0),
                      key=lambda x:x[1],reverse=True)[:16]
    w_model,w_mkt=0.55,0.45
    champ_out=[]
    for t,prob,(lo,hi) in champ_list:
